...
> **方案版本**：V3.8.5  
> **文档状态**：评审整合增强版——微内核架构、按需扩展、轻量敏捷、灵活部署、安全可靠、简单易用、极致性能、运维高效八大原则驱动的落地实现优化版  
> **编制日期**：2026-07-18  
> **整合依据**：《HNB 云原生平台需求规格说明书（核心平台版）V3.1》、《HNB Cloud 开放可组装云原生平台项目落地方案 V3.6》、《HNB Cloud V3.6 方案评审报告与 KubeEdge 边缘计算补充方案》及《HNB Cloud 开放可组装云原生平台项目落地方案 V3.8.2》  
> **核心定位**：以最小平台内核、独立应用市场、统一 OCI 制品存储、可选 AI Extension Plane、可选 Edge Pack、Gateway API 标准化服务网络、能力包、服务蓝图、组合编排和多类型 RuntimeTarget，建设面向传统应用、云原生应用、AI 应用与边缘应用的开放、可组装、轻量、敏捷、高可用企业运行与运营平台。  
> **落地原则**：本版以**微内核架构、按需扩展、轻量敏捷、灵活部署、安全可靠、简单易用、极致性能、运维高效**八大原则作为交付验收的统一标尺，所有能力包、实施阶段和验收项均可追溯到八大原则中的具体条款（见 2.4）。
...
V3.8.5 以 V3.8.2 的完整评审整合版为基础，不再新增功能范围，而是围绕**微内核架构、按需扩展、轻量敏捷、灵活部署、安全可靠、简单易用、极致性能、运维高效**八大落地原则，对全篇的实现路径和落地顺序做一次系统性梳理、收敛和优化。V3.8.2 解决的是“功能和边界要不要做、怎么定义”，V3.8.5 解决的是“在组件和服务数量已经很多的前提下，按什么顺序、以什么颗粒度、用什么判据把它安全地交付出去”。
...
- Gateway API 作为规范而非产品、Edge Pack 作为运行扩展而非第五平面。

V3.8.2 已补齐 Gateway API 标准化和边缘体系的缺口；V3.8.5 进一步解决“方案里能力和组件很多、但缺少一把统一的落地取舍标尺”的问题——即在评审、排期、验收和运维中反复出现的问题：“这个组件是否必须在第一天就上？”“这个能力默认打开还是默认关闭？”“出了故障，谁先看、看什么？”。V3.8.5 用八大原则和能力分级模型（2.4）统一回答这些问题，不改变已确立的架构边界。

### 0.2 V3.8.5 关键修订

| 主题 | V3.8.5 调整 |
|---|---|
| 落地方法论 | 新增“八大原则 × 能力分级”框架（2.4），把微内核、按需扩展、轻量敏捷、灵活部署、安全可靠、简单易用、极致性能、运维高效逐条落到可验证的架构决策和验收判据上 |
| 能力分级 | 将全部能力包和组件划分为 **T0 内核必装 / T1 默认必选 / T2 标准可选 / T3 扩展可选**四级，替代此前“核心/可选”的二元划分，分级结果贯穿实施路线、验收体系和资源规划 |
| 实施路线 | 在阶段 0/MVP/V1/V1.5/V2 每个阶段前增加“进入门槛”和“退出判据”，判据直接引用八大原则的量化指标，避免阶段目标口径漂移（21.6） |
| 简单易用 | 明确“三层模式 + 动态表单 + 场景化向导”是默认体验层，Kubernetes 原生对象一律不作为一线交互界面（17、2.4） |
| 极致性能 | 汇总内核请求延迟、Read Model 刷新、制品直传直取、Gateway 数据面转发和边缘断连收敛等关键路径的性能预算，形成统一的“性能预算表”（2.4.4、24.7） |
| 运维高效 | 将可观测、备份恢复、升级回滚、故障隔离和容量规划的运维动作按“默认内建 / 一键触发 / 需人工介入”重新分类，缩小“需人工介入”的比例（19、21.6） |
| 灵活部署 | 统一 Minimal / Lite HA / Standard HA / Enterprise 四档部署与能力分级的对应关系，避免出现“轻量档位却强依赖重型可选能力”的组合（15.5、24） |
| 验收体系 | 新增“八大原则符合度”验收矩阵，作为 MVP、V1、V1.5、V2 之外的横切验收维度，不新增独立验收阶段（23.2） |
| 文档质量 | 增加 2.4 章节导航条目；历史版本演进关系统一表述为 V3.1 → V3.6 → V3.8 → V3.8.2 → V3.8.5，不改变既有章节编号 |

### 0.3 统一架构口径

V3.8.5 使用以下统一口径，后续章节不得出现冲突表述：
...
7. **Gateway API 是规范而非产品**：HNB 使用 Gateway API 表达 GatewayClass、Gateway、Route 与相关策略，实际控制器和数据面通过 Gateway Provider 接入。
8. **CRD 由集群级治理**：Gateway API CRD 属于高权限集群级资源，由 HNB Cluster Provider 或集群管理员统一安装和升级；业务租户只能管理被授权的 Route。
9. **流量治理分层**：Gateway API 负责标准化入口和路由；API Management 负责消费者、插件、开发者门户和 API 生命周期；Service Mesh 负责网格内治理；AI Gateway 负责模型协议、安全围栏、Token 和成本治理。
10. **所有指标均为设计目标**：节点规模、RPO、RTO、延迟、吞吐和网关容量最终以兼容矩阵、Conformance、压测和故障演练结果为准。

### 0.4 文档阅读导航

| 章节范围 | 主要内容 |
|---|---|
| 1–4 | 执行摘要、关键结论（含 2.4 八大原则与能力分级模型）、产品边界、总体逻辑架构 |
| 5–8 | 最小内核、应用市场、职责分工、接口与事件 |
| 9–11 | 软件包模型、组合编排、Capability/CapabilityPack/Blueprint |
| 12 | AI as Workload / Service / Platform 与 AI 治理 |
| 13–15 | RuntimeTarget、Gateway API/Gateway Provider、EdgeRuntimeTarget、Provider 契约与部署方式 |
...
- KubernetesTarget 以 Gateway API 作为首选统一服务暴露和路由规范，屏蔽不同 Gateway Controller 的资源差异；Ingress 保留为存量兼容接口；
...
V3.8.5 建议将产品划分为三个独立部署、松耦合集成的核心系统，并以统一制品存储平面作为共享基础能力：
...
| 可插拔能力包 | 增强 | 增加市场包、组合包、Gateway Pack、依赖锁和兼容认证 |
| Gateway API | 新增并标准化 | 作为 KubernetesTarget 的首选流量入口 API；CRD 独立治理，控制器与数据面通过 Gateway Provider 可替换接入 |
...
3. **应用交付闭环**：选择版本、参数化、预检、审批、执行、Gateway/Route/TLS 暴露、验证、升级和回滚。
4. **制品存储闭环**：上传、摘要校验、签名关联、复制、缓存、保留、备份、恢复和安全回收。
5. **服务运营闭环**：应用、数据库和中间件的监控、Gateway/Route 状态、流量策略、备份、升级、扩缩容、绑定和删除。
...
本节是 V3.8.5 相对 V3.8.2 的核心增量。此前各版本已经把“做什么、边界在哪里”讲清楚，但组件和服务数量增多后，落地现场反复出现同一类问题：这个组件第一天要不要上、默认开还是关、出问题先看哪里。2.4 用一套统一的原则判据和能力分级，把这些问题的答案固化下来，后续章节的实施路线、验收体系和容量规划都直接引用本节结论，不再各自表述。

#### 2.4.1 八大原则定义与落地判据

| 原则 | 定义（对本方案的具体含义） | 主要落地手段 | 关联章节 | 量化/可验证判据 |
|---|---|---|---|---|
| 微内核架构 | 内核只提供身份、租户、Operation Engine、Read Model、Provider 注册和事件总线，不内置任何具体 CNI/CSI/数据库/中间件/网关实现 | 一切具体能力通过 Provider 和能力包接入；内核代码库与能力包代码库物理分离（20） | 5、14、20 | 内核镜像不依赖任何具体 Provider 镜像；卸载任意非核心能力包，内核可正常启动并通过健康检查 |
| 按需扩展 | 平台的功能边界由已安装的能力包决定，未安装的能力既不消耗资源也不出现在界面上 | CapabilityPack 声明依赖、资源占用和默认开关；Portal 按已安装能力动态渲染菜单和向导（11、17） | 11、17、26 | 关闭 AI Extension Plane 或 Edge Pack 后，平台核心闭环（应用交付、备份、升级、回滚）验收全部通过 |
| 轻量敏捷 | 最小可用形态资源占用低、启动快、迭代周期短；不要求生产环境从第一天就具备全部高可用组件 | Minimal 部署档位、渐进式 HA 升级路径、CompositionRelease 支持小步快跑发布（10、15、24） | 15.5、24.1、21 | Minimal 档位单机可运行；从提交 Release 到租户可用的端到端交付周期在阶段验收中给出实测值 |
| 灵活部署 | 同一套容器化制品可部署到物理机、虚拟机、私有云、公有云和边缘环境，由 RuntimeTarget 和兼容性声明决定实际可用范围 | RuntimeTarget 统一模型、ReleaseManifest 的 targetTypes 声明、Provider 认证等级（13、14） | 13、15 | 同一 Release 在 KubernetesTarget、ContainerEngineTarget 上产出一致的部署结果，差异仅限于已声明的兼容性范围 |
| 安全可靠 | 双重安全门禁（市场供应链 + 平台运行）、发布不可变、Provider 最小权限、数据面自治 | Cosign 签名、SBOM、OCI Referrer、mTLS、多租户隔离、撤销处置（18） | 18、26.5 | 未签名或未通过扫描的制品无法进入 stable 渠道；中心控制面故障期间已运行服务不受影响 |
| 简单易用 | 用户看到的是应用、数据库、中间件、模型服务和解决方案，不是大量 Kubernetes 原生对象 | 统一入口、三层模式（简单/标准/专家）、动态表单、场景化向导（17） | 16、17 | 新用户在向导模式下完成一次标准应用交付无需理解底层 CRD |
| 极致性能 | 关键路径（内核 API、Read Model 查询、制品直传直取、Gateway 数据面、边缘断连收敛）有明确性能预算并可实测验证 | 性能预算表（2.4.4）、直传直取、区域/站点缓存、Gateway Provider 分层认证（19、24.7） | 24.7 | 各关键路径的 P95 延迟、吞吐和收敛时间以压测和演练结果为准，不允许仅以“预计”表述 |
| 运维高效 | 容量、备份、恢复、升级、OTA 和故障演练是产品能力而不是人工脚本；日常运维动作以“默认内建 / 一键触发”为主 | Operation Engine 统一状态机、UsageRecord/MeterEvent、AIOps 分级演进（19、21.6） | 19、21.6 | “需人工介入”类运维动作在验收清单中占比可量化，并随版本演进下降 |

#### 2.4.2 能力分级模型：T0–T3

为避免“核心/可选”二元划分掩盖真实的落地优先级，V3.8.5 将所有能力包和组件统一划分为四级：

- **T0（内核必装）**：身份认证、租户/项目/环境模型、Operation Engine、Read Model、Provider 注册中心、事件总线、审计。不可卸载，不因能力包增减而变化。
- **T1（默认必选）**：应用市场目录、Artifact Storage Minimal Profile、至少一个 KubernetesTarget 或 ContainerEngineTarget、基础 Network/Storage/Secret、Gateway Pack（Gateway API Standard + 一个默认 Gateway Provider）、基础指标日志审计。首个可交付组合（27）即以 T1 为边界。
- **T2（标准可选）**：Lite HA/Standard HA 制品存储档位、多 Gateway Provider 分层认证、AI Access Pack（外部模型 Connector + AI Gateway）、只读 Platform Copilot、ISV 沙箱。按租户和场景选择安装，默认关闭。
- **T3（扩展可选）**：AI Runtime Provider（本地推理/训练关联能力）、Platform AIOps 自动化执行、Edge Pack 全量能力（KubeEdge、设备接入、批量 OTA）、多地域/多集群高级能力。安装前必须完成对应的 POC 和 Conformance 认证。

分级规则：

1. 任何 T2/T3 能力包卸载或未安装，都不得影响 T0/T1 能力的正常运行与验收结果（对应“按需扩展”“数据面自治”）；
2. 能力分级只决定“默认是否安装”，不改变 Provider 可替换、契约治理和安全门禁要求；
3. 阶段性实施路线（21）的每个阶段以能力分级为准入依据，禁止把 T3 能力提前到 MVP 阶段强制交付；
4. 验收体系（23.2）按分级分别设置验收范围，T0/T1 为强制项，T2/T3 为可选交付项。

#### 2.4.3 能力分级清单（关键组件）

| 能力/组件 | 分级 | 主要服务的原则 | 备注 |
|---|---|---|---|
| Operation Engine / Read Model / Provider 注册中心 | T0 | 微内核架构、极致性能 | 内核组成部分，不可卸载 |
| HNB App Market 目录与 Release/CompositionRelease | T1 | 按需扩展、简单易用 | 首个可交付组合必含 |
| Artifact Storage（Minimal + Lite HA） | T1 | 安全可靠、极致性能 | OCI 优先、直传直取 |
| KubernetesTarget + OCI/Helm/Java Provider | T1 | 灵活部署 | 首批 RuntimeTarget |
| Gateway Pack（Gateway API Standard + 默认 Provider） | T1 | 简单易用、灵活部署 | Ingress 仅兼容迁移 |
| 基础指标/日志/审计 | T1 | 运维高效、安全可靠 | 内建，非附加脚本 |
| Artifact Storage（Standard HA） | T2 | 安全可靠、运维高效 | 生产级共享存储与备份 |
| 多 Gateway Provider（Envoy/Cilium/Istio/APISIX/Kong 等） | T2 | 灵活部署 | 按基础设施分层认证 |
| AI Access Pack（外部模型 Connector + AI Gateway） | T2 | 按需扩展、安全可靠 | 不要求 GPU |
| 只读 Platform Copilot | T2 | 简单易用、运维高效 | 不设执行旁路 |
| AI Runtime Provider（本地推理） | T3 | 按需扩展 | 需 GPU/NPU 配额 |
| Platform AIOps 自动化执行 | T3 | 运维高效 | 需前置只读阶段验证 |
| Edge Pack（KubeEdge、设备接入、OTA） | T3 | 灵活部署、安全可靠 | 需完成 POC 与兼容矩阵认证 |

#### 2.4.4 关键路径性能预算（摘要，详见 24.7）

| 关键路径 | 预算方向 | 对应原则 |
|---|---|---|
| 内核 API（租户/Operation 提交） | P95 延迟低、不随集群规模线性劣化 | 极致性能 |
| Read Model 查询（列表/检索） | 不实时遍历集群，缓存刷新周期可配置 | 极致性能、运维高效 |
| 制品直传直取 | 大文件走数据面直连，不经控制面中转 | 极致性能、安全可靠 |
| Gateway 数据面转发 | 由 Gateway Provider 实测认证，不由控制面代理转发 | 极致性能、灵活部署 |
| 边缘断连重连收敛 | 收敛时间可观测、可设定 SLO | 运维高效、安全可靠 |

以上预算为设计目标，最终以兼容矩阵、Conformance、压测和故障演练结果为准（对应 0.3 第 10 条统一口径）。
...
模型训练、微调、数据标注和实验追踪可以作为后续市场产品或外部平台集成，但不进入 V3.8.5 微内核。
...
`EdgeCore` 作为边缘节点上的容器运行底座管理代理，通常以节点级二进制和 systemd 服务运行。V3.8.5 将其定义为容器化硬约束的**显式、受控例外**，条件如下：
...
V3.8.5 将体系结构统一表达为：**一个统一体验层、四个逻辑平面、一个容器执行与连接层、三类主要运行目标**。
...
┌───────────────────────────────────────▼──────────────────────────────────────────────┐
│                           容器执行、连接与 Provider 层                               │
│ cluster-agent │ container-agent │ Gateway Provider │ Edge Provider │ Runtime Driver       │
│ Gateway API Controller/Data Plane │ Helm/Operator │ CNI/CSI │ Security │ OTel │ DB/MW/AI │
└───────────────┬────────────────────────┬────────────────────────┬─────────────────────┘
                │                        │                        │
       ┌────────▼─────────┐     ┌────────▼─────────┐     ┌────────▼─────────────────────┐
...
- Kubernetes 集群中的 GatewayClass、Gateway、Route 和 Policy 状态由对应 Gateway Controller 负责协调，HNB Platform 保存 GatewayProfile、能力快照、业务关联、策略与审计投影；
...
- Gateway API 是 KubernetesTarget 的首选标准服务网络接口，但不是 HNB Core 的编译依赖；CRD、Controller 和数据面均由 Gateway Pack/Provider 管理；
- Gateway API CRD 由集群管理员或 HNB Cluster Provider 统一治理，同一集群只维护一个受控 Bundle；Gateway Controller 不得覆盖未知或更高版本 CRD；
- 一个集群可存在多个 GatewayClass 和不同 Controller，但每个 GatewayClass 必须具有唯一 controllerName、清晰的责任域和认证状态；
- Ingress 仅作为存量兼容和迁移入口，新建平台产品默认生成 Gateway API 资源，不继续扩展厂商 Annotation；
- 普通 Gateway、API Management、Service Mesh 和 AI Gateway 必须分层建模，禁止因产品同时具备多种能力而混淆数据面和权威边界；
...
| 中间件 | 消息队列、缓存、注册配置、任务调度 |
| 网络与流量 | Gateway API Controller、API 网关、Ingress 兼容、Service Mesh、DNS、负载均衡、流量策略 |
...
- 架构：x86_64、arm64、多架构；
- 运行环境：Kubernetes、Container Engine、Edge Runtime；
...
- Gateway API：Bundle 版本、Standard/Experimental Channel、Gateway/Mesh Profile、支持的 Route 类型；
- 网关能力：HTTP/gRPC/TLS/TCP/UDP、BackendTLS、权重分流、请求镜像、重写、重定向、CORS、超时和跨命名空间；
- 网关实现：Controller、数据面、部署模式、Conformance 版本、Core/Extended 特性和商业支持等级。
...
network.policy
network.gateway-api
network.gateway-api.http
network.gateway-api.grpc
network.gateway-api.tls
network.gateway-api.tcp
network.gateway-api.udp
network.gateway-api.backend-tls
network.gateway-api.traffic-split
network.gateway-api.cross-namespace
network.ingress-compat
...
| Container Pack | Kubernetes 纳管、Helm、OCI 应用、基础网络和存储 | 建议默认 |
| Gateway Pack | Gateway API CRD 治理、Gateway Provider、Controller、GatewayClass、Route、TLS、状态和兼容迁移 | Kubernetes 生产建议；实现可替换 |
...
#### 11.3.1 Gateway API 暴露示例

应用 Blueprint 不直接绑定 NGINX、Envoy 或云负载均衡产品，而是声明所需 Gateway API 能力和暴露策略：
...
spec:
  ...
  exposure:
    mode: gateway-api
    gatewayProfileRef: public-standard
    routeKinds: [HTTPRoute]
    tlsRequired: true
    crossNamespaceBackend: deny
  lifecycle:
    required: [provision, expose, observe, delete]
    optional: [canary, mirror, rollback, rotate-certificate]
```

平台根据目标集群的 GatewayCapabilitySnapshot 选择已认证 GatewayClass，将暴露声明渲染为 Gateway API 原生资源。产品发布方不得在 Blueprint 中写死 Controller 专有 Annotation；专有能力必须通过显式 Provider 扩展策略声明。
...
- 平台首先聚焦模型推理和 AI 应用运行，不在 V3.8.5 内核建设通用训练平台；
...
#### 13.2.1 Gateway API 定位与版本原则

Gateway API 是由 Kubernetes SIG Network 维护的独立 CRD 规范，不是从某个 Kubernetes 版本开始自动内置的核心 API。HNB Cloud 将其定位为 KubernetesTarget 的首选南北向服务暴露标准，并为 GAMMA 模式下的东西向服务路由预留能力。

版本治理规则：

1. 生产仅安装 **Standard Channel**，Experimental 只能进入研发沙箱或显式隔离的实验集群；
2. 当前 V3.8.5 认证基线为 **Gateway API v1.5.1 Standard**，但长期不把 Bundle 版本写死在内核；
3. Gateway API 每个发行版以 Bundle 形式提供 CRD，并承诺覆盖最近若干 Kubernetes 次要版本；实际支持范围必须以该 Bundle、Controller 和 HNB 认证矩阵共同确定；
4. Gateway API v1.5 的 TLSRoute v1 CEL 校验要求 Kubernetes 1.31 或更高，因此“安装了 CRD”不等于目标集群具备全部标准能力；
5. Gateway API、Gateway Controller 和数据面必须分别升级、验证和回滚，禁止由多个 Controller 争抢 CRD 所有权；
6. Standard CRD 的升级由集群管理员或 HNB Cluster Provider 执行，租户不得自行安装、降级或切换 Channel。

#### 13.2.2 核心资源与职责模型

```text
GatewayClass（基础设施提供方/平台管理员）
        │ controllerName
        ▼
Gateway（集群/租户网关管理员）
        │ listeners / addresses / TLS / allowedRoutes
        ├── HTTPRoute
        ├── GRPCRoute
        ├── TLSRoute
        ├── TCPRoute
...
BackendTLSPolicy     Gateway 到后端的 TLS 配置
ListenerSet          多团队扩展 Gateway Listener
```
...
| 角色 | 主要资源 | HNB 权限边界 |
|---|---|---|
| 集群基础设施管理员 | Gateway API CRD、GatewayClass、Controller | 安装 Bundle、注册 Controller、维护地址池和基础设施参数 |
| 平台/网关管理员 | Gateway、GatewayProfile、证书引用 | 创建共享或专属网关、Listener、TLS 和 Route 附着策略 |
| 租户/应用团队 | HTTPRoute、GRPCRoute 等 | 仅管理授权 Namespace 和 Gateway Listener 下的 Route |
| 安全管理员 | ReferenceGrant、BackendTLSPolicy、准入策略 | 审批跨命名空间、后端 TLS、证书和高风险路由能力 |
| HNB Operation Engine | ExecutionPlan 与原生资源执行 | 统一预检、审批、创建、观察、回滚和审计，不直接承担流量转发 |

#### 13.2.3 标准能力范围

HNB Gateway Pack 至少覆盖：

- HTTP/HTTPS Host、Path、Header、Query Parameter 和 Method 匹配；
- gRPC Service/Method 路由；
- 权重分流、金丝雀和蓝绿发布；
- 请求镜像、URL 重写、HTTP 重定向；
- 请求/响应 Header 修改、CORS、请求和后端超时；
- TLS 边缘终止、TLS 透传和 BackendTLSPolicy 后端再加密；
- TCP、UDP 和 TLS Route 的按实现认证支持；
- ReferenceGrant 跨命名空间显式授权；
- Gateway/Listener/Route Conditions、Accepted、Programmed、ResolvedRefs 和附着状态；
- Gateway 与 Route 级日志、指标、链路、证书和后端健康观测。

能力分为 Core、Extended 和实现专有扩展。ExecutionPlan 只能使用目标 GatewayCapabilitySnapshot 中已声明并通过认证的能力；页面不得仅因 CRD 字段存在就显示为可用。

#### 13.2.4 HNB Gateway 领域投影

HNB 不复制 Gateway API 的路由语义，只增加面向产品化治理的业务投影：

```text
GatewayProviderRegistration   Provider、Controller、数据面和认证信息
GatewayProfile                公网/内网/边缘/AI等可复用网关规格
GatewayCapabilitySnapshot     Bundle、Channel、Profile、Route 和特性快照
ExposurePolicy                Blueprint 声明的暴露、安全和租户策略
RouteBinding                  ProductInstance 与原生 Route/Gateway 的关联
GatewayConformanceReport      上游与 HNB 专项一致性测试证据
```

示例：

```yaml
apiVersion: network.hnb.io/v1alpha1
kind: GatewayProfile
metadata:
  name: public-standard
spec:
  targetRef: prod-k8s-east
  gatewayClassName: hnb-nginx
  gatewayApi:
    bundleVersion: v1.5.1
    channel: standard
    profiles: [GATEWAY-HTTP]
  routes:
    required: [HTTPRoute, GRPCRoute]
    optional: [TLSRoute, TCPRoute, UDPRoute]
  features:
    required:
      - traffic-split
      - request-mirror
  ...
  security:
    tlsRequired: true
    crossNamespaceDefault: deny
```

#### 13.2.5 Ingress 兼容与迁移

- 新建 Product 和 ServiceBlueprint 默认生成 Gateway API，不新增 Controller 专有 Ingress Annotation；
- 存量 Ingress 可以继续运行，并通过 `network.ingress-compat` 能力声明进入兼容模式；
- 平台可提供 Ingress 到 HTTPRoute 的迁移评估和草案转换，但必须人工核对 Annotation、正则路径、认证、WAF、限流和重写等专有语义；
- 同一域名和端口迁移时必须检测 Ingress 与 Gateway 的监听冲突，采用灰度 DNS、独立 VIP 或分阶段切换；
- 迁移完成后保留原配置和快速回退窗口，禁止在未验证 Controller 等价能力前直接删除 Ingress。
...
EdgeRuntimeTarget 面向节点数量大、资源受限、弱网/断连、设备接入和无人值守场景。V3.8.5 的参考实现采用 KubeEdge：云端部署 CloudCore，边缘节点运行 EdgeCore，通过 WebSocket/QUIC 控制隧道同步资源和状态，MetaManager 在边缘本地持久化元数据。
...
1. **Domain Provider**：应用、数据库、中间件、存储、GPU/NPU、安全、灾备；
2. **Gateway Provider**：Gateway API CRD/Controller、GatewayClass、能力发现、资源渲染、状态观察和 Conformance；
3. **Artifact Provider**：OCI、Helm、JAR/WAR Runtime、模型制品、Manifest；
4. **AI Provider**：模型运行时、外部模型、AI Gateway、Guardrail、评测、知识/向量服务；
5. **Edge Provider**：KubeEdge、边缘节点组、设备、边缘 OTA 和边缘运行适配；
6. **Runtime Driver**：Kubernetes、Container Engine 和 Edge Runtime；
7. **Market Connector**：市场目录、Release、授权和事件同步；
8. **Integration Provider**：IAM、ITSM、CMDB、SIEM、通知；
9. **UI Plugin**：可选页面和组件；
10. **Policy Plugin**：校验、准入和治理规则。
...
### 14.5 Gateway Provider 专项契约

Gateway Provider 不直接绕过平台修改集群，而是负责发现、校验、渲染和观察；所有写入仍由 Operation Engine 通过 Kubernetes Runtime Driver 执行。

```go
type GatewayProvider interface {
    Discover(ctx context.Context, target TargetRef) (*GatewayCapabilitySnapshot, error)
    Validate(ctx context.Context, exposure *ExposurePolicy, snapshot *GatewayCapabilitySnapshot) error
    Render(ctx context.Context, exposure *ExposurePolicy, profile *GatewayProfile) ([]KubernetesObject, error)
    Observe(ctx context.Context, binding RouteBindingRef) (*GatewayRuntimeState, error)
    Diagnose(ctx context.Context, binding RouteBindingRef) (*ArtifactRef, error)
    Conformance(ctx context.Context) (*GatewayConformanceReport, error)
}
```

Provider 必须声明：

- Gateway API Bundle、Channel 和 API Version；
- Controller 名称、版本、controllerName 和部署模式；
- 数据面产品、版本、共享/专属实例模式；
- Gateway 或 Mesh Profile；
- 支持的 Route 类型；
- Core、Extended 与专有特性；
- CRD 管理方式和升级边界；
- TLS、证书、跨 Namespace 和地址分配能力；
- 多租户隔离、HA、容量和故障域；
- 上游 Conformance 与 HNB 专项认证报告。

#### 14.5.1 主要实现产品分类

| 类别 | 代表产品 | 常见数据面 | HNB 推荐定位 |
|---|---|---|---|
| 通用独立网关 | NGINX Gateway Fabric | NGINX OSS/Plus | 默认通用 Gateway Provider 候选，运维门槛低 |
| 通用独立网关 | Envoy Gateway | Envoy | 高级标准网关候选，策略和扩展能力强 |
| 轻量网关 | Traefik Proxy | Traefik | Minimal/Lite 与 k3s 场景 |
| 高性能 L4/L7 | HAProxy Ingress | HAProxy | 已有 HAProxy 体系或 TCP 场景 |
| Envoy 入口 | Contour | Envoy | 已有 Contour 环境兼容 |
| CNI 一体化 | Cilium Gateway API | eBPF + Envoy | 已启用 Cilium 时的网络、安全和 Gateway 一体化 |
| CNI 一体化 | Calico Gateway API | Calico + Envoy Gateway | 已有 Calico/Tigera 环境 |
| Service Mesh | Istio | Envoy | Gateway 与 GAMMA/Mesh 统一治理 |
| Service Mesh | Linkerd | Rust Proxy | 主要用于 GAMMA 东西向能力，按 Mesh Profile 认证 |
| API Management | Kong Gateway | Kong | 消费者、插件、认证、限流和 API 生命周期能力包 |
| API Management | Apache APISIX | APISIX | 高性能动态 API 网关和国内生态 Provider |
| API/AI Gateway | Higress | Envoy/Istio | API 与 AI 流量治理扩展，需独立 HNB Conformance |
| API/AI Gateway | kgateway / Gloo Gateway | Envoy | Gateway API、企业策略、AI/MCP 场景 |
| API Management | WSO2 Gateway | WSO2 Data Plane | 完整 API 生命周期与企业集成 |
| 云厂商托管 | GKE Gateway Controller | Google Cloud LB | GKE RuntimeTarget 原生 Provider |
| 云厂商托管 | AWS Gateway/API 与 Load Balancer Controller | VPC Lattice/ALB/NLB | EKS/AWS 原生 Provider |
| 云厂商托管 | Azure Application Gateway for Containers | Azure 托管 L7 | AKS/Azure 原生 Provider |
| Agent/AI 专项 | agentgateway | Rust Proxy | MCP、A2A、LLM/Agent 流量专项，不替代普通 Gateway |

实现清单和上游 Conformance 状态会随版本变化。产品声称“支持 Gateway API”不等于已通过目标 Bundle、Profile、Route 和 Extended Feature 的完整一致性测试；未出现在上游近期 Conformance 列表中的实现，必须通过 HNB 自有测试后才能标记 Production Ready。

#### 14.5.2 HNB 推荐认证梯队

1. **首批基础认证**：NGINX Gateway Fabric、Envoy Gateway；
2. **基础设施绑定认证**：Cilium Gateway API、Istio，以及 GKE/AWS/Azure 托管实现；
3. **API Management Pack**：APISIX 或 Kong 首选一个完成首期完整认证，其余按客户生态扩展；
4. **AI Gateway 扩展**：Higress、kgateway/agentgateway 等可以作为 AI Gateway Provider 候选，但仍需与普通 GatewayProfile 分开建模；
5. **轻量环境**：Traefik 可作为 Minimal/Lite 可选实现，不作为平台唯一默认。

默认实现不得写死在 HNB Core。`GatewayProfile` 只引用 GatewayClass 和认证能力，切换 Provider 时保持 Product、ServiceBlueprint 和 ExposurePolicy 尽量不变。
...
- 平面间管理 API 通过受控内部 API Gateway 或服务网关连接；该管理入口与业务 Gateway API 数据面相互隔离；
- 不共享运行时 Secret；
...
hnbctl pack enable gateway
hnbctl gateway api install --bundle v1.5.1 --channel standard
hnbctl gateway providers list
hnbctl gateway provider enable nginx-gateway-fabric
hnbctl gateway profile create --file public-standard.yaml
hnbctl gateway conformance run --profile public-standard
hnbctl gateway routes list --target prod-k8s-east
hnbctl gateway migrate ingress plan --namespace app-prod
...
| Gateway API CRD | 不启用或单集群 Standard Bundle | 统一受控 Standard Bundle | 按集群兼容矩阵统一治理 |
| Gateway Controller | 可选 Traefik/单实例实现 | 2+ 副本 NGINX/Envoy Gateway 等 | 多 GatewayClass、专属数据面或云托管实现 |
...
### 18.9 Gateway API 安全与多租户

1. Gateway API CRD、GatewayClass 和 Controller 安装权限仅授予集群管理员或 HNB Cluster Provider；
2. 共享 Gateway 默认通过 `allowedRoutes` 和 Namespace Selector 限制可附着 Route；
3. 跨 Namespace Backend、Secret 或证书引用必须由资源拥有方创建 ReferenceGrant，默认拒绝；
4. Gateway Listener、Hostname、端口和地址冲突必须在预检阶段阻断，禁止“最后写入者生效”；
5. TLS Secret 采用 SecretReference/KMS 管理，租户不能读取共享 Gateway 的私钥；
6. BackendTLSPolicy 的 CA、SAN、Hostname 和客户端证书策略必须经过安全管理员审批；
7. Route 中的 Header 注入、重写、镜像、外部 Backend 和专有插件属于可分级高风险能力；
8. API Key、JWT、OIDC、WAF、消费者和开发者门户不属于 Gateway API Core，应由 API Management/Security Provider 显式提供；
9. Gateway Controller 和数据面使用独立 ServiceAccount、网络策略、PDB、反亲和和资源配额；
10. 租户 Route 状态、流量指标和日志必须带 Tenant/Project/Namespace 标签，禁止跨租户查询。
...
- Agent 连接；
- Provider 健康；
- Gateway API Bundle/CRD 版本、GatewayClass Accepted 状态和 Controller 健康；
- Gateway/Listener/Route 的 Accepted、Programmed、ResolvedRefs、冲突和附着状态；
- Gateway 数据面连接数、请求率、错误率、延迟、重试、超时、TLS 握手和证书有效期；
- 各租户、GatewayProfile、Route、Backend 和状态码维度的流量与容量；
- 部署步骤耗时；
- 市场索引、扫描和镜像同步任务；
- 审计和安全事件。
...
目标：验证容器化硬约束、市场/平台边界、AI Extension Plane 解耦原则，完成 Gateway API/Gateway Provider 基线验证，并完成 Edge Pack 的关键风险概念验证。
...
- JAR/WAR 标准运行时容器验证；
- Agent mTLS；
- Gateway API v1.5.1 Standard CRD 安装、升级保护和版本发现验证；
- NGINX Gateway Fabric 与 Envoy Gateway 双实现 POC，覆盖 HTTPRoute、GRPCRoute、ReferenceGrant、BackendTLSPolicy、权重分流、镜像和回滚；
- Ingress 到 HTTPRoute 的迁移评估、监听冲突和快速回退验证；
- 依赖 DAG 和补偿验证；
...
- 至少两个 Gateway Provider 通过相同 ServiceBlueprint 和 ExposurePolicy 完成部署，切换实现时业务发布模型保持不变；
- 租户 Route 无法越权附着未授权 Gateway 或跨 Namespace 引用 Backend/Secret；
...
- 租户、项目、环境；
- KubernetesTarget；
- Gateway API Schema、GatewayProfile、ExposurePolicy 和 GatewayCapabilitySnapshot；
- 基础 HTTP/HTTPS 暴露，优先使用 Gateway API；存量 Ingress 进入兼容模式；
...
- Provider 认证；
- Gateway Pack 生产可用：统一 Standard CRD、至少一个默认 Gateway Provider、GatewayClass/Profile 管理、HTTPRoute/GRPCRoute、TLS、ReferenceGrant、BackendTLSPolicy、流量分割与可观测；
- 第二个 Gateway Provider 完成可替换性认证和故障回退演练；
- 升级和回滚；
- 数据备份和恢复；
- 应用运行时安全；
- 离线部署；
...
- GPU 能力包；
- 多集群部署；
- API Management Pack：APISIX 或 Kong 完成消费者、认证、限流、插件和审计能力认证；
- Service Mesh/GAMMA Provider 按需认证 Istio/Cilium 等实现；
- 应用组合升级；
...
- Karmada；
- 全局流量；
- 多集群 GatewayProfile、全局入口、DNS/GSLB 与跨地域流量策略；
- Gateway API GAMMA 东西向路由和 Mesh Profile 统一治理；
- 高级灾备；
- 兼容性遥测和智能推荐；
- 行业解决方案市场；
- Partner/ISV 认证体系；
...
### 21.6 分阶段进入门槛与退出判据（V3.8.5 新增）

各阶段的能力范围本身不变，本节新增的是：每个阶段开始前必须满足什么条件（进入门槛），结束时必须证明什么才能进入下一阶段（退出判据）。判据统一引用 2.4.1 的八大原则和 2.4.2 的能力分级，避免阶段目标在评审中被口头放宽。

| 阶段 | 进入门槛 | 退出判据（对应原则） |
|---|---|---|
| 阶段 0 | 内核（T0）设计评审通过，Provider 契约（14）冻结 | 内核可独立启动并通过健康检查，不依赖任何 T1+ 能力包（微内核架构） |
| MVP | 阶段 0 全部退出判据通过；27 章首个交付组合的 T1 能力包清单冻结 | 27 章闭环全流程跑通；关闭任一 T2/T3 能力包不影响该闭环（按需扩展、安全可靠） |
| V1 | MVP 验收通过；Standard HA 档位资源规划（24）评审通过 | Lite HA/Standard HA 高可用故障演练通过；关键路径性能预算（2.4.4）实测达标（极致性能、运维高效） |
| V1.5 | V1 验收通过；Edge Pack 完成阶段 0 POC 与兼容矩阵认证（T3 准入要求） | Edge Pack 专项验收（23.2.1）全部通过；启用 Edge Pack 不影响已通过的 MVP/V1 验收项（灵活部署、按需扩展） |
| V2 | V1.5 验收通过；多地域/多集群方案完成容量和性能压测 | 26.15 落地方法论治理需求全部满足；八大原则符合度矩阵（23.2.2）连续两个版本达标 | 
...
- Gateway API Bundle/CRD 安装升级、Controller Conformance、GatewayClass/Gateway/Route 状态和多租户权限测试；
- HTTP/gRPC/TLS/TCP/UDP、权重、镜像、重写、BackendTLS、ReferenceGrant 和 Ingress 迁移测试。

### 23.2 平台专项验收（延续 V3.8.2 基线）
...
| GW-ACC-01 | Gateway API CRD 由集群级统一治理，业务租户无权安装、降级或切换 Channel |
| GW-ACC-02 | Gateway API Bundle、Kubernetes、Controller 和数据面版本均可发现并进入兼容矩阵 |
| GW-ACC-03 | 相同 ServiceBlueprint/ExposurePolicy 可分别通过 NGINX Gateway Fabric 与 Envoy Gateway 等两个 Provider 完成部署 |
| GW-ACC-04 | HTTPRoute 支持 Host、Path、Header、Method、Query 匹配以及权重分流、重写、重定向和请求镜像 |
| GW-ACC-05 | GRPCRoute 完成 gRPC Service/Method 路由；不支持的 Route/Feature 在预检阶段明确拒绝 |
| GW-ACC-06 | TLS 终止、TLS 透传和 BackendTLSPolicy 按认证能力工作，证书轮换不中断或可控回退 |
| GW-ACC-07 | 未获 ReferenceGrant 授权的跨 Namespace Backend/Secret 引用被拒绝，授权后状态可追踪 |
| GW-ACC-08 | 共享 Gateway 的 allowedRoutes 和 Namespace Selector 能阻止租户越权附着 Route |
| GW-ACC-09 | Gateway、Listener 和 Route 的 Accepted、Programmed、ResolvedRefs、冲突和附着状态可在 HNB Portal 查看 |
| GW-ACC-10 | Controller 或单个数据面实例故障时按 HA 设计继续服务，升级和回滚经过 Operation 并保留审计 |
| GW-ACC-11 | Ingress 迁移工具可识别无法等价转换的 Annotation、监听冲突和高风险规则，并生成可审阅草案 |
| GW-ACC-12 | AI Gateway 与普通业务 Gateway 数据面、凭据、路由和观测指标相互隔离 |
...
#### 23.2.2 八大原则符合度验收矩阵（V3.8.5 新增）

本矩阵是横切验收维度，不新增独立验收阶段，而是要求上述各专项验收项在通过测试的同时能明确指出其服务于哪些原则；下表给出每条原则的代表性判据入口，具体测试用例复用 23.1–23.2.1。

| 编号 | 原则 | 判据入口 | 判定方式 |
|---|---|---|---|
| PRIN-ACC-01 | 微内核架构 | 卸载全部 T2/T3 能力包后重新执行 CTN-ACC-01～03 | 内核启动与核心 API 全部通过 |
| PRIN-ACC-02 | 按需扩展 | 关闭 AI Extension Plane / Edge Pack 后执行 MKT-ACC、CMP-ACC 全套 | 核心闭环验收不受影响 |
| PRIN-ACC-03 | 轻量敏捷 | Minimal 档位下完成一次端到端 Release 交付并记录耗时 | 交付周期有实测数据且逐版本可比 |
| PRIN-ACC-04 | 灵活部署 | 同一 Release 在两种以上 RuntimeTarget 完成部署对比 | 部署结果一致，差异仅限已声明兼容范围 |
| PRIN-ACC-05 | 安全可靠 | 复用 SEC-ACC-01、AI-ACC-05、EDGE-ACC-09 | 未签名制品拒绝、租户隔离、断连仍安全 |
| PRIN-ACC-06 | 简单易用 | 新用户在向导模式完成一次标准应用交付且不接触底层 CRD | 任务完成且无需平台工程师协助 |
| PRIN-ACC-07 | 极致性能 | 复用 24.7 性能指标压测结果 | 各关键路径 P95 指标达到既定预算 |
| PRIN-ACC-08 | 运维高效 | 统计“需人工介入”类运维动作占比 | 相对上一版本占比下降或不高于既定阈值 |
...
- Gateway Provider 通过目标 Bundle/Profile/Route 的上游或 HNB Conformance，所有声明的 Extended Feature 均有测试证据；
- Gateway API CRD 升级、Controller 滚动升级、数据面故障和证书轮换演练通过。
...
### 24.9 Gateway Pack 容量与高可用

Gateway 容量不能只按 Controller 副本估算，应区分控制面和数据面：

| 档位 | Controller | 数据面 | 建议场景 |
|---|---|---|---|
| Gateway Lite | 1–2 副本 | 1–2 实例，共享 Gateway | 开发、小型生产，需明确单故障域限制 |
| Gateway Standard | 2–3 副本，PDB/反亲和 | 至少 2 实例，可按 GatewayProfile 隔离 | 一般企业生产 |
| Gateway Enterprise | 独立控制面或多 GatewayClass | 多可用区、专属/共享混合、云托管或分片 | 集团、多租户、大流量和强隔离 |

容量压测至少覆盖：并发连接、新建连接率、HTTP RPS、gRPC 长连接、TLS 握手、证书数量、Route/Gateway 资源规模、配置收敛时间、Controller 重启、数据面滚动升级、后端异常、权重切换和观测开销。商业 SLA 必须基于具体 Controller、数据面、基础设施和配置实测，不得由“支持 Gateway API”直接推导。
...
| Gateway API CRD 多方管理 | Controller 覆盖、降级或安装不同 Channel 导致集群级故障 | CRD 单一所有者、Standard Channel、升级护栏和版本发现 |
| “支持 Gateway API”名不副实 | 只实现少量字段或未通过目标版本 Conformance | 按 Bundle/Profile/Route/Extended Feature 认证并保存报告 |
| 网关产品边界混乱 | 普通流量、API 管理、Mesh 和 AI 流量全部塞入一个产品 | GatewayProfile、API Management Pack、Mesh Provider、AI Gateway 分层 |
| Ingress 迁移语义丢失 | Annotation、正则、认证、WAF 或重写无法等价转换 | 迁移评估、人工审阅、灰度 DNS/VIP、保留回退窗口 |
| 共享 Gateway 租户越权 | Route 附着或跨 Namespace 引用不受控 | allowedRoutes、Namespace Selector、ReferenceGrant、RBAC 和审计 |
| Gateway 数据面成为单点 | Controller 正常但代理实例、VIP 或证书故障 | 数据面多副本、多可用区、PDB、证书轮换和故障演练 |
...
### 26.12 Gateway API 与服务网络

| 编号 | 优先级 | 需求 |
|---|---|---|
| GW-01 | P0 | KubernetesTarget 必须支持以 Gateway API 作为首选标准服务暴露接口，Ingress 仅作为兼容迁移能力。 |
| GW-02 | P0 | Gateway API CRD 必须由集群管理员或 HNB Cluster Provider 统一管理，租户不得安装、降级或切换 Channel。 |
| GW-03 | P0 | 生产环境只允许 Standard Channel；Experimental 必须位于隔离实验环境并明确标识。 |
| GW-04 | P0 | 平台维护 Gateway API Bundle、Kubernetes、Controller、数据面和 Feature 的兼容矩阵。 |
| GW-05 | P0 | 提供 GatewayProviderRegistration、GatewayProfile、GatewayCapabilitySnapshot、ExposurePolicy 和 RouteBinding。 |
| GW-06 | P0 | Gateway Provider 必须声明 Profile、Route 类型、Core/Extended 特性、CRD 管理方式和 Conformance 证据。 |
| GW-07 | P0 | Gateway Provider 只负责发现、校验、渲染和观察，所有资源写入必须经过 ExecutionPlan、Operation 和 Runtime Driver。 |
| GW-08 | P0 | 至少支持 GatewayClass、Gateway、HTTPRoute、GRPCRoute、ReferenceGrant 和 BackendTLSPolicy。 |
| GW-09 | P1 | TLSRoute、TCPRoute、UDPRoute、ListenerSet 和 GAMMA 按 Provider 认证能力启用。 |
| GW-10 | P0 | 支持 Host、Path、Header、Query、Method 匹配，权重分流、镜像、重写、重定向、Header 修改和超时。 |
| GW-11 | P0 | 共享 Gateway 必须使用 allowedRoutes、Namespace Selector、RBAC 和 Tenant Context 实现租户隔离。 |
| GW-12 | P0 | 跨 Namespace Backend、Secret 和证书引用必须通过 ReferenceGrant 或规范允许的显式授权。 |
| GW-13 | P0 | Gateway/Listener/Route 状态、Controller 健康、数据面指标和证书状态必须进入 Read Model 与可观测体系。 |
| GW-14 | P1 | 提供 Ingress 迁移评估、转换草案、冲突检查、灰度切换和快速回退能力。 |
| GW-15 | P0 | 普通 Gateway API、API Management、Service Mesh 和 AI Gateway 必须分层建模并隔离凭据、数据面和审计。 |
| GW-16 | P1 | 首批至少认证两个可替换 Gateway Provider，其中一个作为默认通用实现。 |
| GW-17 | P1 | Controller 和数据面必须支持 HA、滚动升级、证书轮换、故障诊断和容量压测。 |
| GW-18 | P1 | 未通过目标 Bundle/Profile/Route Conformance 的实现不得标记为 Production Ready。 |
...
### 26.15 落地方法论治理（V3.8.5 新增）

| 编号 | 优先级 | 需求 |
|---|---|---|
| METH-01 | P0 | 每个能力包在市场发布时必须声明能力分级（T0/T1/T2/T3），未声明分级的能力包不得进入 stable 渠道。 |
| METH-02 | P0 | 实施路线每个阶段（21）的进入门槛和退出判据必须显式引用 2.4.1 中的原则判据，不得使用未量化的表述。 |
| METH-03 | P1 | 平台控制台提供“已安装能力分级视图”，展示当前环境的 T0/T1/T2/T3 能力清单及其资源占用。 |
| METH-04 | P1 | 每次大版本验收报告包含 23.2.2 八大原则符合度矩阵的实测结果，不得仅以“符合设计目标”一笔带过。 |
| METH-05 | P2 | 运维手册按“默认内建 / 一键触发 / 需人工介入”标注每类运维动作，并在版本间跟踪“需人工介入”占比变化。 |

## 27. 推荐首个可交付产品组合

首个版本建议只交付以下组合，验证完整闭环而非追求数量。本组合即 2.4.2 中 **T0（内核）+ T1（默认必选）** 的具体清单；组合之外的能力均为 T2/T3，按需在后续阶段安装，不影响本组合的验收结果。
...
+ Gateway Pack（Gateway API Standard + 默认 Gateway Provider）
+ 基础指标、日志和审计
...
4. **Java Web 标准解决方案**：Java 应用 + PostgreSQL + 缓存/消息 + Gateway API HTTPRoute/TLS + 监控；
...
→ 多包依赖部署
→ 自动生成连接 Secret 和服务绑定
→ 通过 GatewayProfile 生成 Gateway/HTTPRoute/TLS 暴露并完成权重灰度
→ 查看日志、指标和告警
→ 备份数据库
→ 升级应用版本
→ 回滚
...
31. **Gateway API 优先**：新建 Kubernetes 服务入口默认使用 Gateway API，Ingress 仅保留兼容和迁移能力。
32. **规范与实现分离**：Gateway API 资源模型不绑定 NGINX、Envoy、Cilium、Istio、APISIX、Kong 或云厂商实现。
33. **网关分层治理**：普通 Gateway、API Management、Service Mesh 和 AI Gateway 各自承担明确职责，不能以单一产品能力重叠为由破坏边界。
34. **CRD 集中治理**：Gateway API Bundle 和 Channel 由集群级单一责任方维护，租户只管理授权 Route。
35. **能力以 Conformance 为准**：产品声明和 CRD 字段不能替代版本化一致性测试、兼容矩阵和实测证据。
36. **分级先于取舍（V3.8.5 新增）**：任何能力包上线前必须先归入 T0/T1/T2/T3 分级（2.4.2），再决定是否进入某个交付阶段，不允许绕过分级直接凭经验排期。
37. **原则可验证（V3.8.5 新增）**：微内核架构、按需扩展、轻量敏捷、灵活部署、安全可靠、简单易用、极致性能、运维高效八大原则均对应 2.4.1 中的量化判据，评审和验收以判据为准，不接受笼统的“符合原则”结论。
38. **运维动作占比可下降（V3.8.5 新增）**：以“默认内建 / 一键触发 / 需人工介入”为标尺持续跟踪运维动作分布，“需人工介入”占比应随版本演进下降，不应因能力增多而被动上升。
...
- Gateway API 当前认证基线：截至 2026-07-18，稳定 Bundle 为 v1.5.1；生产使用 Standard Channel。Gateway、GatewayClass、HTTPRoute 为 GA 核心资源，ReferenceGrant、BackendTLSPolicy、TLSRoute、ListenerSet 等按当前 Standard 版本和 Provider Conformance 使用；
- Gateway API 兼容原则：每个 Bundle 的 Kubernetes 支持范围按官方最近版本策略和实现矩阵确定；Gateway API v1.5 的 TLSRoute v1 CEL 校验需要 Kubernetes 1.31 或更高。HNB 不以“集群能安装 CRD”替代完整能力认证；
- Gateway Provider 首批建议：NGINX Gateway Fabric 和 Envoy Gateway；Cilium、Istio、APISIX/Kong、Higress/kgateway 及云厂商实现按基础设施、API 管理、Mesh 和 AI 场景分层认证；
- KubeEdge 参考基线：V3.8.5 设计面向 KubeEdge v1.22–v1.23 能力范围，实施时必须固定具体版本并维护 CloudCore、EdgeCore、Kubernetes、CRD 和 Provider 兼容矩阵；
...
13. Gateway API 项目与稳定版本：https://github.com/kubernetes-sigs/gateway-api/releases
14. Gateway API 介绍与资源模型：https://gateway-api.sigs.k8s.io/docs/introduction/
15. Gateway API 版本与 Channel：https://gateway-api.sigs.k8s.io/docs/concepts/versioning/
16. Gateway API CRD 管理：https://gateway-api.sigs.k8s.io/guides/crd-management/
17. Gateway API API Overview：https://gateway-api.sigs.k8s.io/docs/concepts/api-overview/
18. Gateway API Conformance：https://gateway-api.sigs.k8s.io/docs/concepts/conformance/
19. Gateway API 实现清单：https://gateway-api.sigs.k8s.io/docs/implementations/list/
20. Gateway API v1.5 实现矩阵：https://gateway-api.sigs.k8s.io/docs/implementations/versions/v1.5/
21. HTTPRoute：https://gateway-api.sigs.k8s.io/reference/api-types/httproute/
22. GRPCRoute：https://gateway-api.sigs.k8s.io/reference/api-types/grpcroute/
23. ReferenceGrant：https://gateway-api.sigs.k8s.io/reference/api-types/referencegrant/
24. BackendTLSPolicy：https://gateway-api.sigs.k8s.io/reference/api-types/policy/backendtlspolicy/
