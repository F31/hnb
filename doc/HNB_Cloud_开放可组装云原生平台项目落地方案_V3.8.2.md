...
> **方案版本**：V3.8.2  
> **文档状态**：评审整合增强版——容器化交付、统一应用市场、生产级制品存储、AI 增强、云边协同与 Gateway API 标准化服务网络  
> **编制日期**：2026-07-18  
...
V3.8.2 以 V3.8 的完整评审整合版为基础，对 Gateway API 的版本治理、资源模型、产品实现、Provider 契约、能力包、实施路线和验收体系进行横向贯通。它不是在文末简单追加一个“网关产品清单”，而是把统一服务暴露与流量治理纳入平台既有的 Release、ExecutionPlan、Operation、Capability 和 RuntimeTarget 体系。
...
V3.8 已补齐 V3.6 的边缘体系缺口；V3.8.2 进一步解决“文档提及 Gateway API，但缺少统一版本基线、实现选型、职责边界、产品化模型和验收标准”的问题，同时保持微内核、四平面和 Provider 可替换原则不变。
...
| 服务网络标准 | 将 Gateway API 定位为 KubernetesTarget 的首选南北向流量入口规范，并为 GAMMA 东西向路由预留能力；Ingress 仅作为兼容迁移路径 |
| Gateway API 版本 | 当前认证基线采用 Gateway API v1.5.1 Standard Channel；Bundle、Kubernetes、Controller 与数据面版本必须进入兼容矩阵，不把 CRD 等同于 Kubernetes 内置 API |
| Gateway 实现 | 新增 Gateway Provider 和 Gateway Pack；支持 NGINX Gateway Fabric、Envoy Gateway、Cilium、Istio、Kong、APISIX、Higress 及云厂商实现的分层认证，不绑定单一产品 |
| 网关职责边界 | 区分 Gateway API 标准入口、完整 API Management、Service Mesh 与 AI Gateway；普通业务流量不得误经 AI Gateway，Gateway Controller 不得绕过 Operation Engine |
...
V3.8.2 建议将产品划分为三个独立部署、松耦合集成的核心系统，并以统一制品存储平面作为共享基础能力：
...
模型训练、微调、数据标注和实验追踪可以作为后续市场产品或外部平台集成，但不进入 V3.8.2 微内核。
...
`EdgeCore` 作为边缘节点上的容器运行底座管理代理，通常以节点级二进制和 systemd 服务运行。V3.8.2 将其定义为容器化硬约束的**显式、受控例外**，条件如下：
...
V3.8.2 将体系结构统一表达为：**一个统一体验层、四个逻辑平面、一个容器执行与连接层、三类主要运行目标**。
...
- 平台首先聚焦模型推理和 AI 应用运行，不在 V3.8.2 内核建设通用训练平台；
...
2. 当前 V3.8.2 认证基线为 **Gateway API v1.5.1 Standard**，但长期不把 Bundle 版本写死在内核；
...
EdgeRuntimeTarget 面向节点数量大、资源受限、弱网/断连、设备接入和无人值守场景。V3.8.2 的参考实现采用 KubeEdge：云端部署 CloudCore，边缘节点运行 EdgeCore，通过 WebSocket/QUIC 控制隧道同步资源和状态，MetaManager 在边缘本地持久化元数据。
...
- KubeEdge 参考基线：V3.8.2 设计面向 KubeEdge v1.22–v1.23 能力范围，实施时必须固定具体版本并维护 CloudCore、EdgeCore、Kubernetes、CRD 和 Provider 兼容矩阵；
