...
/**
 * ClusterDrawer — 右侧滑入抽屉（UI 规范 V2.6 抽屉规范）。
 * 遮罩 + 焦点圈定 + ESC 关闭 + 底部固定按钮区；内容经默认 slot 传入。
 */
...
<template>
  <Teleport to="body">
    <div v-if="modelValue" class="hnb-drawer-layer" @click.self="close">
      <div
        ref="bodyEl"
        class="hnb-drawer"
        role="dialog"
      ...
      >
        <header class="hnb-drawer__header">
          ...
          <button class="hnb-drawer__close" type="button" :disabled="busy" :aria-label="t('resource.clusterMgmt.common.close')" @click="close">×</button>
        </header>
        ...
        <footer class="hnb-drawer__footer">
          ...
          <HNBButton :loading="busy" @click="emit('confirm')">
            {{ t('resource.clusterMgmt.common.confirm') }}
          </HNBButton>
        </footer>
      </div>
    </div>
