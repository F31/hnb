...
- 提供提交前和评审前统一执行的本地检查入口；GitHub 仅用于代码托管。
...
- 相同提交上的重复本地检查应产生一致结论。
- 检查耗时和失败原因进入评审证据；不新增运行时指标、备份或灾备要求。
- 资源预算为单次本地门禁，目标是在常规开发环境中 60 秒内完成 18 个主规格及活动 change 校验。

## Rollout and Rollback

- 以独立脚本验证现有基线，并将命令和结果纳入提交评审证据。
- 门禁启用前必须确认当前 18 个主规格全部通过严格校验。
- 若门禁因 OpenSpec CLI 兼容问题阻塞所有提交，必须保留失败记录并通过修复 change 更新批准版本，不得静默跳过。
- 本 change 无不可逆步骤。
...
- **GIVEN** 一个仅修改文档的提交，**WHEN** 在相同固定环境重复执行检查，**THEN** 两次产生一致结果且总耗时不超过批准预算。
