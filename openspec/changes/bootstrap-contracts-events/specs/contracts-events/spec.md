...
仓库 SHALL 将版本化 OpenAPI、Protobuf、事件和 Manifest Schema 作为跨进程公共契约的唯一真源，并 SHALL 提供本地与持续集成一致调用的 lint、兼容检查、Go/TypeScript 生成和生成物漂移门禁；门禁 SHALL 固定工具版本，SHALL 拒绝同一主版本内的破坏性变化、手工修改生成物以及包含 Secret、目标凭据或大文件正文的公共消息定义。
