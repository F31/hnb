# HNB Web Console UI / UX 设计规范 V2.6

> 版本：V2.6　|　发布日期：2026-08-03　|　密级：内部
>
> 本版基于《HNB Web Console 微内核插件化架构设计 V3.6》和《UI / UX 设计规范 V2.5》进一步升级，在性能治理、缓存策略和运行时优化方面进行了专项增强，重点解决 **Schema 引擎运行时性能瓶颈**。包括：DataSource 请求缓存与去重、PageRenderer 单体 computed 拆分、ComponentRegistry props 校验缓存、ConditionEvaluator 实例复用、Schema 变更 diff 加载，以及 ConditionTerm 字段清理。
...
| 维度 | V2.0 | V2.5 | V2.6 |
|---|---|---|---|
| 扩展范围 | 主要覆盖动态菜单、插件装载、主题和多租户 | 扩展到导航、页面布局、表单、表格、详情、仪表盘、操作、字典和状态模型 | 同 V2.5 |
| 页面实现 | 页面主要由插件代码实现 | 标准页面优先由服务端 `PageSchema` 驱动，复杂页面由注册组件或插件实现 | 同 V2.5 |
| 组件使用 | 依赖 UI Kit，但缺少动态组件约束 | 增加 `Component Registry`，后端只能引用受信组件类型和受控属性 | 增加 `validateProps` 缓存校验（§8.2） |
| 数据访问 | 描述按需请求，但未形成统一契约 | 增加 `DataSource`、Query、Mutation、分页、轮询和实时订阅契约 | 增加请求缓存和 in-flight 去重（§13.5） |
| 动态操作 | 未定义统一操作模型 | 增加导航、API、异步任务、工作流、下载和批量操作的统一 `ActionSchema` | 同 V2.5 |
| 安全边界 | 明确前端不是安全边界 | 进一步禁止服务端下发任意 JavaScript、HTML、模板表达式和远程脚本 | 同 V2.5 |
| 版本治理 | 主要依赖插件版本 | 增加 `apiVersion`、`schemaVersion`、`revision`、`minShellVersion` 和兼容策略 | 同 V2.5 |
| 扩展生命周期 | 插件加载与激活分离 | 增加"注册—校验—发布—通知—刷新—回滚"的 UI 元数据生命周期 | 同 V2.5 |
| 测试治理 | 未形成体系 | 增加 Schema Lint、契约测试、视觉回归、A11y 和动态扩展验收清单 | 增加缓存命中率、重算隔离、条件类型检查（§21.1） |
| **页面渲染性能** | — | — | **拆分单体 computed 为 per-region computed，单 region 变化不影响其他 region（§7.2）** |
| **数据源缓存** | — | 仅定义 `cacheKey` 接口但未使用 | **强制实现响应缓存 + in-flight 请求去重（§13.5）** |
| **条件求值** | — | 每次渲染创建多个实例 | **复用单例实例减少 GC 压力（§4.4）** |
| **Schema 变更** | — | 全量注销并重新加载所有 region | **diff 新旧 schema，仅加载变化 region（§7.4）** |
| **条件类型** | — | 包含 9 个字段，5 个未使用 | **移除未使用的条件类型（§4.3）** |
| **性能预算** | — | 首屏可交互时间 ≤ 2.5 秒 | **首屏可交互时间 ≤ 1.5 秒（§15.5）** |
...
6. **【V2.6 新增】** DataSource 层必须实现请求缓存和去重，避免同一数据源被多次请求。
7. **【V2.6 新增】** PageRenderer 必须拆分单体 computed 为 per-region computed，避免单 region 数据变化拖累全部 region。
8. **【V2.6 新增】** ComponentRegistry 必须对 props 校验结果做 memoization，避免重复校验。
9. **【V2.6 新增】** ConditionEvaluator 应复用实例，避免每次渲染创建新对象。
10. **【V2.6 新增】** Schema 变更时应 diff 新旧 schema，仅重建变化 region 的数据，不触发全量重刷。
11. **【V2.6 新增】** 移除运行时未使用的 ConditionTerm 字段，减少 Schema 体积和解析开销。
12. **【V2.6 新增】** 所有缓存必须考虑多租户隔离，缓存键包含 tenant/context 标识。
...
| 缓存优先 |  **【V2.6】**  DataSource 层必须实现响应缓存和请求去重，减少重复网络开销 |
| 最小重算 |  **【V2.6】**  页面渲染器必须按 region 粒度拆分响应式依赖，避免级联重算 |
| 零冗余校验 |  **【V2.6】**  组件 props 校验必须缓存结果，相同输入跳过校验 |
...
- `permission` — 权限检查
- `feature` — Feature Flag 检查
- `license` — License 检查
- `capability` — 能力检查
- `context` — 上下文字段存在性检查

**V2.6 变更**：移除以下未使用的条件类型：`role`、`exists`、`fieldValue`、`notEmpty`、`resourceState`。Schema 中如包含这些字段，引擎应忽略并记录调试日志，但不拒绝渲染。
...
- 单个区块失败：区块级隔离，不影响整页（**V2.6 强化**：per-region computed 保证单 region 数据变化不会触发其他 region 重算）；
- 核心导航或根页面失败：进入最小安全页面。

**【V2.6 新增】** `ConditionEvaluator` 在页面渲染周期内应复用单例实例，避免每次条件求值或 region 渲染时创建新对象。
...
1. **【必须】** 每个 region 使用独立的 computed 属性，禁止在单个 computed 中编排所有 region 的组件解析、props 校验和条件求值。单 region 数据变化不得触发其他 region 重算。
2. **【必须】** region 的 `runtimeProps`（包含 data/loading 等动态属性）不得在渲染器层每次创建新对象引用，应使用 per-region computed 返回稳定引用，避免触发 Vue 子组件不必要的 patch。
3. **【建议】** region 数据源查找应使用 Map（O(1)）替代数组线性查找（O(n)）。
...
**V2.6 新增 Schema 变更加载规则**：

1. **【必须】** 当 `PageSchema` 发生变更时，渲染器必须 diff 新旧 schema 的 regions，仅对新增的 region 或 `dataSource` 发生变化的 region 重新加载数据。
2. **【必须】** 未发生变化的 region 复用已有数据和状态，不得触发重新请求。
3. **【建议】** 数据源注册（`registerEndpoint`/`registerDataSource`）是幂等的，不应在 schema 变更时全量清空后重建。
...
**V2.6 新增**：`validateProps` 方法必须实现 memoization 缓存。

**【必须 V2.6】** `validateProps` 必须按 `(type, stableJSON(props))` 缓存校验结果，相同 props 跳过递归校验，避免每次渲染重复执行。注册/注销/清除组件时清空缓存。
...
**V2.6 新增**：`DataSourceManager` 必须实现以下缓存机制：

### 13.6.1 响应缓存

按 `(dataSourceId + contextKey + stableJSON(params))` 缓存响应结果，同参数同上下文的请求直接返回缓存，不发起网络请求。context/tenant 切换时递增 generation 并清空缓存。

### 13.6.2 in-flight 请求去重

同一数据源同一参数的并发请求共享同一个 Promise，避免重复发起 HTTP 请求。

### 13.6.3 缓存键

缓存键必须包含：
- `dataSourceId`
- `contextKey`（tenantId 或 context signature）
- 稳定序列化参数（`stableJSON(params)`）

### 13.6.4 缓存失效

- `invalidateContext()` 调用时递增 `generation`，清空响应缓存和 in-flight 队列。
- `clear()` 调用时同时清空缓存。
- 上下文切换后的迟到响应通过 `generation` 比对丢弃。

### 13.6.5 缓存隔离

**【必须】** 不同 tenant/context 的缓存通过 `contextKey` 隔离，禁止跨租户复用缓存。
...
| 指标 | V2.5 目标 | V2.6 目标 |
|---|---|---:|---:|
| Shell 初始 JS（gzip） | ≤ 300KB，业务插件不计入 | ≤ 300KB |
| 首屏可交互时间 | 内网标准环境 ≤ 2.5 秒 | **≤ 1.5 秒** |
| 导航接口 P95 | 缓存命中 ≤ 100ms，回源 ≤ 500ms | 同 V2.5 |
| Page Schema P95 | 缓存命中 ≤ 100ms | 同 V2.5 |
| 路由切换反馈 | 100ms 内出现反馈，主要内容 1 秒内开始渲染 | 同 V2.5 |
| 单个插件 Bundle | 建议 gzip ≤ 500KB，超出需拆分 | 同 V2.5 |
| 单个 UI Schema | 建议 ≤ 200KB | **≤ 100KB** |
| DataSource 缓存命中率 | 未定义 | **≥ 90%（同参数 5 分钟内重复请求）** |
| per-region 重算隔离 | 未定义 | **单 region 数据变化不应触发其他 region 的组件解析和 props 校验** |
...
| **缓存命中率** | **【V2.6 新增】** DataSource 缓存命中率 ≥ 90% |
| **重算隔离** | **【V2.6 新增】** 单 region 数据变化不触发其他 region 重算 |
| **条件类型** | **【V2.6 新增】** Schema 中不包含已移除的 `role`/`exists`/`fieldValue`/`notEmpty`/`resourceState` 字段 |
| **Props 校验缓存** | **【V2.6 新增】** 相同 props 不触发重复校验 |
...
- 是否定义兼容和回滚方案？
- 是否满足性能预算？
- **是否已实现 DataSource 缓存和请求去重？【V2.6 新增】**
- **是否已拆分 per-region computed？【V2.6 新增】**
- **是否已实现 validateProps 缓存？【V2.6 新增】**
- **是否已复用 ConditionEvaluator 实例？【V2.6 新增】**
- **是否已实现 Schema 变更 diff 加载？【V2.6 新增】**
- **是否已清理未使用的 ConditionTerm 字段？【V2.6 新增】**
...
| **V2.6** | **2026-08-03** | **性能治理专项：DataSource 缓存与去重、PageRenderer per-region computed、ComponentRegistry props 校验缓存、ConditionEvaluator 实例复用、Schema diff 加载、条件类型清理** |
