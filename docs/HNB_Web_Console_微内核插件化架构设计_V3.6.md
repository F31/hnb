# HNB Web Console 微内核插件化架构设计 V3.6

> 基于 V3.5 架构设计及页面开发沟通，进一步明确 HNB 控制面的缓存边界：数据库仍是菜单、路由、插件和权限的唯一权威来源；Navigation Service 采用“进程内 L1 缓存 + NATS JetStream KV L2 共享快照 + 数据库回源”的分层模型。NavigationResponse 在 JetStream KV 中默认保留 10 分钟，可配置范围为 5～30 分钟；TTL 仅用于清理旧版本和事件遗漏兜底，正常一致性依靠复合版本键、Transactional Outbox 和 NATS 变更事件。Redis 不作为 HNB 微内核的默认依赖，仅在高频分布式缓存、原子限流、复杂数据结构或第三方组件强依赖时按需引入，2026-07-28。
...
10. **分层缓存而非通用缓存替代** — Navigation Service 优先使用 30 秒左右的进程内 L1 缓存，并以 NATS JetStream KV 保存默认 10 分钟的 L2 共享快照；数据库始终作为权威来源。NATS KV 只用于小对象、低频更新、可重建的控制面状态，不承担通用高频缓存职责。
...
│  │ IAM │ Navigation Service（L1）│ Plugin Registry │ Capability │License│  │
│  │ API Server │ Extension Manager │ Marketplace │ Operation Worker    │  │
│  └───────────────┬───────────────────────┬─────────────────────────────┘  │
│                  │                       │                                │
│        ┌─────────▼─────────┐   ┌─────────▼─────────┐                      │
│        │ PostgreSQL/MySQL  │   │ NATS JetStream    │                      │
│        │ 权威数据源（L3）  │   │ KV（L2）+ 事件流 │                      │
│        └───────────────────┘   └───────────────────┘                      │
│                                                                          │
│  ┌────────────────────────── Plugin Layer ─────────────────────────────┐  │
...
→ Navigation Service 计算快照标识并先查询进程内 L1 缓存
→ L1 未命中时查询 NATS JetStream KV L2 共享快照
→ L2 未命中或校验失败时查询数据库、生成导航树并回写 L2/L1
→ 权限/插件/菜单变更提交后通过 NATS 事件发布新版本
...
- **Navigation Service** 在服务端完成联合过滤、排序、树构建和版本生成，并采用进程内 L1 缓存与 NATS JetStream KV L2 快照降低重复计算；两级缓存均可从数据库重建。
...
1. 使用与当前用户、租户、空间和版本完全匹配的 Last Known Good 导航快照；或
...
→ Navigation Service
→ L1 进程内缓存（默认 30 秒）
→ L2 NATS JetStream KV（默认 10 分钟）
→ 未命中时查询 Database + IAM + Plugin Registry + License/Feature Policy
→ 返回权限过滤后的导航树并回写 L2/L1
```
...
菜单来源单一，不等于插件 Bundle 只能有一种交付方式。V3.6 保留：
...
→ COMMIT
→ Outbox Publisher 发布 NATS 版本变更/失效事件
```
...
### 6.3 NavigationResponse 分层缓存模型

数据库仍是菜单、路由、插件、租户授权和权限配置的唯一权威来源。缓存只保存可重建的计算结果，不得反向成为配置事实来源。

```text
请求 /api/v1/navigation/menus
        │
        ▼
L1：Navigation Service 进程内缓存
默认 TTL：30 秒，可配置 15～60 秒
        │ 未命中
        ▼
L2：NATS JetStream KV 共享快照
默认 MaxAge：10 分钟，可配置 5～30 分钟
        │ 未命中/校验失败
        ▼
L3：PostgreSQL/MySQL 权威数据源
        │
        ├── 计算 NavigationResponse
        ├── Best Effort 写入 L2
        └── 写入 L1 后返回
```
...
```text
KV Bucket: HNB_NAVIGATION_CACHE
Key: nav.{sha256(userId|tenantId|spaceId|permissionVersion|pluginCatalogVersion|navigationVersion|licenseVersion|locale)}
Value: NavigationCacheEntry(JSON/MessagePack)
History: 1
MaxAge/TTL: 默认 10 分钟，允许 5～30 分钟
Replicas: 单节点部署为 1；生产高可用部署建议与 NATS 集群副本策略一致
Storage: 可重建的小规模环境可使用 Memory/R1；标准生产建议 File/R3
```

> “短期 10 分钟”是缓存对象在没有主动失效时的最大自然存活期，不代表菜单变更允许延迟 10 分钟。正常情况下，权限、菜单、插件、License 或 Feature Flag 变化后会立即提升版本，新请求使用新的复合键，旧快照不再命中并由 TTL 自动回收。

`NavigationCacheEntry` 至少包含：

```text
cacheKeyHash
userIdHash
context(tenantId/spaceId/locale)
versions(permission/pluginCatalog/navigation/license)
etag
generatedAt
expiresAt
...
```text
1. 计算当前上下文和版本对应的复合键
2. 查询 L1；命中且版本、上下文、有效期完全匹配则返回
3. 查询 L2；命中且校验通过则写入 L1 并返回
4. L2 未命中、过期或校验失败时查询数据库
5. 生成 NavigationResponse 后先返回主流程，再 Best Effort 回写 L2
6. L2 写入失败只记录指标和告警，不影响本次导航响应
```

约束：

- KV Key 使用复合信息的哈希值，避免在 Key 中暴露 userId、tenantId 等敏感标识；
- 快照不得跨用户、跨租户或跨空间复用；后续若使用 `permissionSetHash` 共享快照，必须保证权限集合、租户、空间、License 和插件版本完全一致；
- 版本变化后生成新 Key，禁止为失效而扫描删除海量旧 Key；旧值由 MaxAge 自动回收；
- L1 默认 30 秒，主要吸收同一实例内的重复读取；L2 默认 10 分钟，主要减少多实例间的重复数据库计算；
- NATS KV 不可用时直接回源数据库，不得阻断 Navigation API；
- 对高风险权限操作，后端业务 API 必须实时授权，不能依赖导航快照；
- 对刚完成的权限撤销、租户停用等强一致性变更，可让首个刷新请求绕过 L1/L2 直接回源数据库，或采用面向 JetStream Leader 的一致性读取策略，避免依赖可能由 follower/mirror 响应的直接读取；
- 记录 L1/L2 命中率、数据库回源次数、快照生成耗时、写入失败、快照年龄和缓存键数量；
- 为 Bucket 设置最大容量和单值大小限制，防止异常用户维度或版本维度造成无界增长。

### 6.4 NATS 版本变更与失效事件

数据库变更与 NATS 发布不能假设为同一事务。插件、菜单、权限或 License 发生变化时，采用 Transactional Outbox 保证最终发布：

```text
数据库事务：修改资源 + bump version + 写入 outbox
→ 事务提交
→ Outbox Publisher 发布 JetStream 事件
→ Navigation Service 实例清理 L1 微缓存
→ API Gateway/通知服务通过 SSE 或 WebSocket 通知 Web Console
→ Shell 触发 navigation:updated 并重新拉取 Navigation API
```

建议 Subjects：

```text
hnb.navigation.updated.{tenantId}
hnb.navigation.invalidate.{tenantId}
hnb.permission.updated.{tenantId}
hnb.plugin.catalog.updated
hnb.license.updated.{tenantId}
```

事件必须包含 `eventId`、`tenantId`、资源类型、旧版本、新版本和发生时间；消费者按 `eventId` 幂等处理。浏览器不直接连接 NATS，所有通知由后端网关转换为受认证的 SSE/WebSocket 消息。
...
### 6.6 NATS KV 的适用边界与 Redis 引入策略

NATS JetStream KV 适合 HNB 控制面中“小对象、低频更新、跨实例共享、需要版本或 Watch、可从数据库重建”的状态：

| 数据类型 | 建议 |
|---|---|
| NavigationResponse、插件目录快照 | 适合使用 L1 + NATS KV |
| 权限版本、导航版本、License/Feature Flag 最新状态 | 适合使用 NATS KV 或 NATS 事件 |
| Capability 最新状态、操作任务最新摘要 | 有条件适合，详细历史仍进入数据库或事件流 |
| 高频业务 API 响应、大规模随机热点 Key | 不优先使用 NATS KV |
| 实时指标、日志、链路数据 | 不使用 NATS KV，进入 Prometheus/时序库、日志系统和链路系统 |
| 排行榜、集合运算、滑动窗口、高频原子计数 | 不适合，按需考虑 Redis 或专用组件 |

Redis 当前不作为 HNB 微内核的强制依赖。只有出现以下明确需求时才引入：

1. **高频分布式共享缓存**：共享缓存访问量和延迟目标明显超出 L1 + NATS KV 的合理范围，并需要 LRU/LFU 淘汰；
2. **同步分布式限流或配额扣减**：需要滑动窗口、Token Bucket、原子计数或 Lua/事务式多键操作；
3. **复杂数据结构**：明确需要 Hash、Set、Sorted Set、Bitmap 等高频原子操作；
4. **第三方组件强依赖**：API Gateway、工作流或其他扩展明确要求 Redis；
5. **压测证据充分**：通过基准测试确认数据库、L1 和 NATS KV 已成为真实瓶颈，而非基于预期提前增加组件。

引入 Redis 时应作为可选基础服务或具体插件依赖，不应让 Shell、Navigation Service 和最小部署档强制依赖 Redis。Redis 故障也不得影响数据库权威数据和 NATS 控制事件链路。
...
```text
角色/权限变更
→ IAM bump permissionVersion 并写入 Transactional Outbox
→ Outbox Publisher 发布 hnb.permission.updated.{tenantId}
→ 新版本复合键自然绕过旧 NATS KV 快照，旧值由 TTL 回收
→ 后端通知通道触发 Web Console 重新获取菜单
→ 当前路由重新校验
→ 无权限时进入 /403 或安全首页
```
...
9. bump pluginCatalogVersion/navigationVersion 并写入 Transactional Outbox
10. 提交后发布 NATS 版本变更事件，旧 KV 快照由版本键隔离并按 TTL 回收
```
...
→ bump navigationVersion 并写入 Transactional Outbox
→ 发布 NATS 导航变更事件；新版本键绕过旧 KV 快照
→ 前端重新拉取导航
→ Router Manager 卸载路由
...
| 档位 | 组件 | 场景 |
|---|---|---|
| 最小 | Shell + Dashboard + IAM + Navigation Service + 数据库 + 单节点 NATS/JetStream | POC、演示、轻量运维 |
| 标准 | 最小档 + Application + Container + Resource + System + NATS JetStream KV/事件流 | 生产核心运维 |
| 完整 | 标准档 + Service + AI + Marketplace + Harbor + Operator Manager + 高可用 NATS 集群 | 企业级全功能 |
...
nats:
  ...
  jetstream:
    ...
    storage: file
  navigation:
    kvBucket: HNB_NAVIGATION_CACHE
    cacheTTL: 10m
    history: 1
    eventStream: HNB_CONTROL_EVENTS
    subjects:
      - hnb.navigation.>
      - hnb.permission.>
      - hnb.plugin.catalog.updated
...
navigationService:
  ...
  cache:
    enabled: true
    type: nats-jetstream-kv
    bucket: HNB_NAVIGATION_CACHE
    ttl: 10m
    l1MemoryTTL: 30s
    l1MaxEntries: 2000
    kvMaxBytes: 1Gi
...
- 不为导航快照单独部署 Redis；优先使用进程内 L1 + 平台已有 NATS JetStream KV，减少基础设施种类和运维面。Redis 仅在经过压测验证的高频缓存、限流或复杂数据结构场景中按需启用。
- 小规模环境可采用单节点 NATS + JetStream 文件存储；若关闭导航 KV，Navigation Service 仍可通过数据库、ETag 和默认 30 秒的 L1 微缓存工作。
- 标准和多副本环境启用 NATS JetStream KV，默认 MaxAge 为 10 分钟，并按平台 NATS 高可用策略配置副本；TTL 只负责旧版本回收，版本事件负责及时刷新。
...
```text
Navigation API 正常      → 优先返回 L1/L2 有效快照，未命中则返回数据库实时结果
L1 不可用或被清空        → 直接查询 NATS KV 或数据库，无服务中断
NATS KV 不可用           → 回源数据库，KV 写回降级为 Best Effort 并告警
NATS 事件流不可用         → API 继续服务，依靠版本号和条件请求发现变化
数据库短暂不可用         → 仅返回上下文、版本和有效期严格匹配的 NATS KV 快照
数据库与 NATS 均不可用    → 前端使用严格匹配的 Last Known Good 快照
...
│   ├── repository/                  # 数据库访问
│   ├── cache/                       # L1 进程缓存 + NATS JetStream KV L2 Adapter
│   ├── version/                     # ETag 与版本失效
│   └── events/                      # NATS Subjects、Outbox Publisher、navigation:updated
├── plugin-registry/
│   ├── catalog/
│   ├── registration/
│   ├── compatibility/
...
| 项目 | V3.5 | V3.6 |
|---|---|---|
| **缓存定位** | NATS KV 保存短期导航快照，但“短期”与使用边界不够明确 | 明确 L1 + NATS KV + 数据库三层模型，NATS KV 只承载可重建控制面快照 |
| **默认 TTL** | 配置示例为 15 分钟，正文给出 5～30 分钟范围 | 标准默认 10 分钟，可配置 5～30 分钟；L1 默认 30 秒 |
| **一致性机制** | 版本键、ETag、Outbox 和事件刷新 | 强调 TTL 仅是回收与漏事件兜底，正常刷新由版本提升和 NATS 事件完成 |
| **缓存读取路径** | 主要描述 KV → 数据库 | 明确 L1 → L2 JetStream KV → L3 数据库的读取与 Best Effort 回写顺序 |
| **缓存隔离** | 复合版本键绑定身份和上下文 | 进一步约束禁止跨用户/租户/空间复用，并允许在严格等价条件下按权限集合哈希共享 |
| **容量治理** | 有 TTL 和版本键 | 增加 L1 条目上限、KV Bucket 容量、单值大小、键数量和快照年龄指标 |
| **NATS 适用边界** | 以减少 Redis 依赖为主 | 明确适合小对象、低频更新、共享状态；不用于实时指标、排行榜和高频复杂原子操作 |
| **Redis 决策** | 不作为导航缓存依赖 | 不作为微内核默认依赖；仅在高频缓存、限流、复杂数据结构或第三方强依赖时按需引入 |
| **权限安全** | 菜单快照绑定权限版本 | 明确高风险业务操作始终实时后端授权，导航缓存永远不是安全边界 |
| **部署配置** | NATS KV TTL 15m、L1 15s | 默认 NATS KV 10m、L1 30s，并增加 L1/KV 容量限制示例 |
...
| **Phase 2** | 完成权限版本、插件目录版本、ETag、L1 缓存、NATS JetStream KV、Outbox 和版本变更事件 | 默认 L1 30 秒、L2 10 分钟的多租户导航闭环 |
...
8. 加入 ETag、默认 30 秒 L1、默认 10 分钟 NATS JetStream KV、Transactional Outbox 和版本变更事件
...
4. 新功能通过 Extension 安装注册插件、菜单、路由、权限和能力，完成后提升版本并通过 NATS 发布变更事件，Shell 无需修改。
...
8. Navigation Service 使用默认 30 秒 L1 与默认 10 分钟 NATS JetStream KV L2，并绑定身份、上下文和版本；TTL 仅清理旧版本，权限或插件变化通过版本提升、Transactional Outbox 和 NATS 事件主动刷新。
...
15. Redis 不属于 HNB 微内核的默认依赖；仅当高频共享缓存、原子限流、复杂数据结构或第三方强依赖出现并经压测验证后，才作为可选组件引入。
16. V3.6 的核心目标是：保持前端和控制面基础设施轻量，在复用 NATS 的同时明确其缓存边界，避免把 JetStream KV 误用为通用 Redis 替代品。
...
- Redis Data Types（仅用于可选场景边界参考）：<https://redis.io/docs/latest/develop/data-types/>
- Redis Eviction（仅用于可选缓存场景参考）：<https://redis.io/docs/latest/develop/reference/eviction/>
