...
> **方案版本**：V3.6  
> **文档状态**：容器化交付、统一应用市场、生产级制品存储与 AI 增强版  
> **编制日期**：2026-07-17  
> **评审依据**：《HNB 云原生平台需求规格说明书（核心平台版）V3.1》及《HNB Cloud 开放可组装云原生平台项目落地方案 V3.5》  
> **核心定位**：以最小平台内核、独立应用市场、统一 OCI 制品存储、可选 AI 能力平面、能力包、服务蓝图、组合编排和容器运行目标，建设面向传统应用、云原生应用与 AI 应用的开放、可组装、轻量、敏捷、高可用企业运行与运营平台。

---

## 0. V3.6 修订说明

V3.6 在 V3.5“全部容器化运行 + 独立应用市场 + 生产级统一制品存储”的基础上，增加人工智能增强架构。AI 不进入 HNB Core 的强依赖，而是通过独立容器化部署的 `HNB AI Extension Plane`、AI Provider、能力包和市场产品接入，使平台同时具备以下三类价值：

1. **AI as Workload**：统一承载大模型、语音、视觉、Embedding、Rerank、RAG 和 Agent 等 AI 工作负载；
2. **AI as a Service**：统一提供模型端点、AI 网关、内容安全、评测、向量服务和调用治理；
3. **AI for Platform**：使用 Platform Copilot 和 AIOps 提升查询、诊断、容量分析、风险识别和受控运维效率。

V3.6 将整体架构明确为四个独立但协同的逻辑平面：

- **应用市场目录与发布平面**：管理产品、模型、AI 应用、分类、版本、渠道、授权和组合定义；
- **统一制品存储与分发平面**：管理镜像、Chart、JAR/WAR、模型制品、评测包、SBOM、签名、复制、缓存和完整性；
- **HNB Cloud 运行治理平面**：管理租户、运行目标、GPU/NPU、策略、部署执行、生命周期和审计；
- **HNB AI Extension Plane**：管理模型接入、推理服务、AI 网关、AI 治理、评测、可观测、Copilot 和 AIOps。

| 修订项 | V3.6 调整 |
|---|---|
| 产品定位 | 从传统应用与云原生服务运营平台扩展为传统应用、云原生应用和 AI 应用的统一运行与运营平台 |
| AI 架构边界 | 新增可选 AI Extension Plane；AI 组件不编译进入微内核，整体可不安装、独立升级和故障隔离 |
| AI 能力分层 | 明确 AI as Workload、AI as a Service、AI for Platform 三层价值 |
| AI 能力包 | 新增 AI Access Pack、AI Runtime Pack、AI Governance Pack 和 AIOps Pack |
| AI 资源模型 | 新增 AIProvider、ModelArtifact、ModelService、ModelEndpoint、PromptTemplate、GuardrailPolicy、EvaluationSuite、AIUsageRecord 等对象 |
| AI 网关 | 独立承担 HTTP/SSE/WebSocket、模型适配、路由、限流、Fallback、安全围栏、用量和成本统计，不承担平台资源部署 |
| AI 应用市场 | 新增模型、AI 运行时、AI 数据服务、AI 安全、AI 评测、Agent 和 AI 解决方案分类与标签 |
| 模型制品 | 模型权重、Tokenizer、推理配置、Prompt、Guardrail 和评测数据优先以 OCI Artifact 管理，也允许引用外部受信模型仓库 |
| Platform Copilot | 支持自然语言查询、诊断和 ExecutionPlan 草案生成；不得绕过权限、策略、审批、Operation 和审计 |
| AIOps | 按摘要、异常关联、根因建议、受控自动修复四级渐进建设，输出必须包含证据和置信度 |
| AI 治理 | 增加模型许可、来源、评测、Prompt 注入、数据泄漏、缓存隔离、内容安全和租户级 Token/成本治理 |
| 轻量部署 | Minimal 可仅连接外部模型，无 GPU；本地模型、向量数据库和 AIOps 均按需安装 |
| 实施与验收 | 更新实施路线、团队、容量、风险和 AI-ARCH/AI-RUN/AI-GW/AI-GOV/AI-OPS 专项需求与验收 |

V3.6 延续 V3.5 的统一制品存储、高可用、高能效、简单接入和高效运维设计，原有 `ART-STO-*` 要求继续有效。
...
- 已运行的数据面不依赖市场、AI 服务或中心控制面的持续可用。

V3.6 建议将产品划分为三个独立部署、松耦合集成的核心系统，并以统一制品存储平面作为共享基础能力：
...
| AI 辅助平台 | 新增 | Copilot/AIOps 只生成解释、建议或受控计划，不成为事实源和执行旁路 |
...
7. **智能运营闭环**：数据采集、异常关联、证据检索、诊断建议、审批确认、Operation 执行和效果验证。
...
模型训练、微调、数据标注和实验追踪可以作为后续市场产品或外部平台集成，但不进入 V3.6 微内核。
...
┌──────────────────────────────────────────────────────────────────────────────┐
│                               统一访问体验                                  │
│ HNB Portal │ Tenant Portal │ AI Workspace │ CLI │ OpenAPI │ SDK │ 第三方门户 │
└────────────────────────────────┬─────────────────────────────────────────────┘
                                 │
     ┌───────────────────────────┼───────────────────────────────┐
     │                           │                               │
┌────▼────────────────────┐ ┌────▼──────────────────────┐ ┌─────▼────────────────────┐
│ HNB App Market          │ │ HNB Cloud Platform       │ │ HNB AI Extension Plane  │
│ 产品与模型发布平面       │ │ 资源与运行治理控制面      │ │ AI 服务与智能运营扩展     │
│                         │ │                          │ │                          │
│ Catalog/Release/Channel │ │ Tenant/Project/Env       │ │ AI Gateway               │
│ Model/Artifact/License  │ │ RuntimeTarget/Cluster    │ │ Model Service Manager    │
│ Composition/Entitlement │ │ Policy/Quota/Approval    │ │ AI Runtime Provider      │
│ Signature/SBOM/Eval     │ │ Operation/ExecutionPlan  │ │ Guardrail/Evaluation     │
│ Publish/Review/Promotion│ │ Provider/Graph/Audit     │ │ Copilot/AIOps/AI OTel    │
└────┬────────────────────┘ └────┬──────────────────────┘ └─────┬────────────────────┘
     │ Catalog/Release API       │ Agent/Provider API            │ AI API/Event/Provider
     └───────────────────────────┼───────────────────────────────┘
                                 │
┌────────────────────────────────▼─────────────────────────────────────────────┐
│                    HNB Artifact Storage 与分发平面                           │
│ OCI Registry │ Model Artifact/External Ref │ Token │ Referrer │ Mirror/Cache │
│ Local/PVC（非 HA）│ S3（HA）│ 企业对象存储 │ HA PostgreSQL Metadata         │
└────────────────────────────────┬─────────────────────────────────────────────┘
                                 │
┌────────────────────────────────▼─────────────────────────────────────────────┐
│                              容器执行面                                     │
│ Cluster Agent │ Container Agent │ Helm │ Operator │ Kubernetes API │ OCI    │
│ CNI │ CSI │ Gateway │ DB/MW Operator │ AI Runtime │ GPU/NPU │ OTel │ Security│
└────────────────────────────────┬─────────────────────────────────────────────┘
                                 │
┌────────────────────────────────▼─────────────────────────────────────────────┐
│                            容器运行基础设施                                 │
│ 物理机 │ 虚拟机 │ 私有云 │ 公有云 │ 边缘 │ Kubernetes/Podman │ GPU/NPU     │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
| 一级分类 | 二级分类示例 |
|---|---|
| 应用 | Web 应用、移动后端、数据应用、行业应用、AI 应用、Agent 应用 |
...
| 基础能力 | 网络、存储、GPU/NPU、证书、DNS、负载均衡 |
...
市场负责 AI Release 的不可变版本和发布门禁；平台与 AI Extension Plane 共同完成目标能力、资源、评测和运行策略预检。
...
- 平台首先聚焦模型推理和 AI 应用运行，不在 V3.6 内核建设通用训练平台；
...
4. **Runtime Driver**：Kubernetes 和 Container Engine；
5. **Market Connector**：市场目录、Release、授权和事件同步；
6. **Integration Provider**：IAM、ITSM、CMDB、SIEM、通知；
7. **UI Plugin**：可选页面和组件；
8. **Policy Plugin**：校验、准入和治理规则。
...
无论底层是物理机还是虚拟机，平台、市场与启用的 AI Extension Plane 组件始终以容器方式运行。AI 能力可以附加到任意部署形态，但不得改变该形态的基础 HA 定义。
...
目标：验证容器化硬约束、市场/平台边界和 AI Extension Plane 解耦原则。
...
18. **AI 可选增强**：无 AI 能力时平台仍具备完整应用和服务运营闭环。
19. **AI 不替代事实和规则**：模型只能辅助理解和建议，权威状态与强制规则保持确定性。
20. **模型与应用同等治理**：模型、Prompt、Guardrail、评测和端点均需版本、权限、审计和生命周期。
21. **Copilot 不设执行旁路**：所有变更统一经过平台 API、审批和 Operation。
22. **从外部模型和只读场景起步**：先获得用户价值，再逐步建设本地推理、AI 治理和受控 AIOps。
